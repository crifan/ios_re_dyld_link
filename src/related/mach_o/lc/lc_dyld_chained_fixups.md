...
* `dyld`中的`load command`
  * `LC_DYLD_CHAINED_FIXUPS`（和`LC_DYLD_EXPORTS_TRIE`）
    * iOS 13中就新增了这个`load command`，但是没广泛使用
    * 到了iOS 15，iOS的app编译期间加了参数`-fixup_chains`，则就会用上这套新的机制
      * 导致
        * 旧的`class-dump`失效了
      * 变化
        * `LC_DYLD_INFO(_ONLY)` 没了，被替代为`LC_DYLD_CHAINED_FIXUPS`和`LC_DYLD_EXPORTS_TRIE`
    * 包含内容
    ...
    * 布局layout
      * ![macho_layout_chained_fixups](../../../assets/img/macho_layout_chained_fixups.png)
    * 导出的Trie
      * 举例
        * ![exported_trie_tree_1](../../../assets/img/exported_trie_tree_1.png)
        * ![exported_trie_tree_2](../../../assets/img/exported_trie_tree_2.png)
    * 对应定义
      * 值的定义
        ```c
        ...
        * Apple开源源码：
          * xnu
            * https://github.com/apple-oss-distributions/xnu/archive/xnu-8792.61.2.tar.gz
              * `EXTERNAL_HEADERS/mach-o/fixup-chains.h`
...
* [fixup-chains.h](https://opensource.apple.com/source/dyld/dyld-852.2/include/mach-o/fixup-chains.h.auto.html)
